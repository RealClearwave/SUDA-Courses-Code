...
2. **中缀表达式求值**：通过输入以“#”结尾的字符串形式的中缀表达式，计算其结果。支持加、减、乘、除及括号。
3. **后缀表达式求值**：通过输入空格分隔、以“#”结尾的后缀表达式，计算其结果。
...
**表达式处理器类**：
- `evaluateInfix(const char *expression)`: 求值中缀表达式。
- `evaluatePostfix(const char *expression)`: 求值后缀表达式。
- `precedence(char op)`: 返回运算符的优先级。
- `applyOperation(double a, double b, char op)`: 对两个数执行运算符操作。
- `readNumber(const char *&expr)`: 从表达式中读取数字。

### 2. 程序运行界面设计

**交互式Shell**：
- 提供Shell界面，允许用户输入中缀或后缀表达式。
- 根据输入表达式格式（是否含空格）自动判断使用中缀或后缀求值方法。
...
#### 基本操作：
1. **EvaluateInfix(ExpressionProcessor &E, const char expression[])**  
   计算中缀表达式并返回结果。

2. **EvaluatePostfix(ExpressionProcessor &E, const char expression[])**  
   计算后缀表达式并返回结果。

3. **Precedence(ExpressionProcessor &E, char op)**  
   返回操作符`op`的优先级。

4. **ApplyOperation(ExpressionProcessor &E, double a, double b, char op)**  
   计算`a op b`的结果，并返回。

5. **ReadNumber(ExpressionProcessor &E, const char &expr[])**  
   从表达式字符串中读取一个数字，并将指针移到数字后的第一个字符。

### 3. 设计思路

顺序栈类采用数组来存储数据，操作数和运算符分别使用两个栈来进行中缀表达式求值。后缀表达式则通过扫描元素并对操作数入栈、操作符执行运算的方式求解。Shell功能允许用户动态输入表达式并获取计算结果。
...
1）**功能验证**：  
所有基本功能测试均通过，包括中缀和后缀表达式的解析和求值。程序能够正确处理加减乘除运算及括号优先级。在输入多种格式的表达式后，程序输出的计算结果与手工计算结果一致，表明核心算法逻辑正确。

2）**表达式格式的兼容性**：  
程序能够兼容中缀和后缀两种表达式格式，并能识别以空格分隔的后缀表达式及带括号的中缀表达式。这一特性提升了程序的使用灵活性，使得用户可以选择不同的表达方式进行计算。

### 2. 性能分析

1）**时间复杂度**：  
中缀表达式的求值算法需要处理整个表达式，并根据操作符的优先级动态调整操作数栈和运算符栈。由于每个字符最多只入栈出栈一次，该算法的时间复杂度为O(n)，其中n是表达式的长度。后缀表达式求值仅需单次遍历，时间复杂度同样为O(n)，因此程序能够高效地处理较长的表达式。
...
在表达式处理器部分，我们分别实现了对中缀和后缀表达式的求值。对于中缀表达式，我们运用了两个栈来分别存储操作数和运算符，并且在运算符入栈时根据其优先级进行判断。对于后缀表达式，我们则是采用了直接扫描操作数入栈、遇到运算符弹出计算的方式。
